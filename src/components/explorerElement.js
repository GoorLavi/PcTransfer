...
import { convertSlashesToBackSlashes, combinePath } from '../utiles/filesUtiles';
...
export default class explorerElement extends Component {
    ...
    render() {
        ...
        const footer = itemSelected ?
            <div>{nameJsx}<Glyphicon className="element-checked-icon" glyph="ok"/></div> : nameJsx;


        return (
            <Col className="explorer-col" xs={3} onClick={() => this.onClick()}
